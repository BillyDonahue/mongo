...
public:
    ...
    Status append(const Event& event) override {
        ...
        using TagInt = std::underlying_type_t<logv2::LogTag::Value>;
        auto tag = logv2::LogTag{
            static_cast<logv2::LogTag::Value>(static_cast<TagInt>(logTagValue) |
                                              static_cast<TagInt>(_tag))};

        if (_warnOnLargeMessages && event.isTruncatable() && message.size() > maxSizeKB * 1024) {
            logv2::detail::doLog(
                0,
                logv2::LogSeverity::cast(event.getSeverity().toInt()),
                logv2::LogOptions{event.getComponent(), _domain, tag},
