...
class TransactionParticipant {
    ...
    public:
        ...
        std::optional<SharedPromise<void>> _exitPreparePromise;
...
public:
    ...
    public:
         ...
         * valid values are std::nullopt (meaning no autocommit was specified) and false (meaning
        ...
        Timestamp prepareTransaction(OperationContext* opCtx,
                                     std::optional<repl::OpTime> prepareOptime);
         ...
         * hasn't executed, returns std::nullopt.
         *
         * Throws if the session has been invalidated or the active transaction number doesn't
         * match.
         */
        std::optional<repl::OplogEntry> checkStatementExecuted(OperationContext* opCtx,
                                                                 StmtId stmtId) const;
    ...
    private:
        std::optional<repl::OpTime> _checkStatementExecuted(StmtId stmtId) const;
     ...
     * time of the stable timestamp. Returns std::nullopt if there are no active transactions, or an
     * error if it fails.
     */
...
private:
    ...
    struct ObservableState {
        ...
        std::optional<TxnResources> txnResourceStash;
        ...
        std::optional<Date_t> transactionExpireDate;
