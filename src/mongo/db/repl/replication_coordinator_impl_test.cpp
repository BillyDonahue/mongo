...
struct OpTimeWithTermOne {
    ...
    operator std::optional<OpTime>() const {
        return OpTime(timestamp, 1);
    }
...
protected:
    ...
    std::pair<SharedClientAndOperation, stdx::future<std::optional<Status>>> stepDown_nonBlocking(
        ...
        auto task = stdx::packaged_task<std::optional<Status>(PromisedClientAndOperation)>(
            [=](PromisedClientAndOperation operationPromise) -> std::optional<Status> {
...
TEST_F(ReplCoordTest, ConcurrentStepDownShouldNotSignalTheSameFinishEventMoreThanOnce) {
    ...
    // exclusive task.
    const auto opCtx = makeOperationContext();
    std::optional<ReplicationStateTransitionLockGuard> transitionGuard;
