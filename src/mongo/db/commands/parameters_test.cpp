...
||||||| parent of 7fae559219... SERVER-46801 stacktrace logging cleanup/consolidation
=======
/**
 *    Copyright (C) 2018-present MongoDB, Inc.
 *
...
||||||| parent of bf2599d5e9... SERVER-46801 tabular LogComponent defs
=======
/**
 *    Copyright (C) 2018-present MongoDB, Inc.
 *
...
public:
    ...
    void assertThrows(const std::string& json, ErrorCodes::Error err, const std::string& reason) {
        auto checker = [=](const DBException& ex) {
            ASSERT_EQUALS(ex.code(), err);
            ASSERT_EQUALS(ex.reason(), reason);
        };
        ASSERT_THROWS_WITH_CHECK(doParse(json), DBException, checker);
    }
};
...
TEST_F(ParseLogComponentSettings, FailNonNumeric) {
    assertThrows(R"({"verbosity": "not a number"})",
                 ErrorCodes::BadValue,
                 "Expected verbosity to be a number, but found string");
}

TEST_F(ParseLogComponentSettings, FailBadComponent) {
    assertThrows(R"({"NoSuchComponent": 2})",
                 ErrorCodes::BadValue,
                 "Invalid component name NoSuchComponent");
}
...
TEST_F(ParseLogComponentSettings, MultiFailBadComponent) {
    assertThrows(
        ...
        ErrorCodes::BadValue,
        "Invalid component name No Such Component");
}
...
>>>>>>> 7fae559219... SERVER-46801 stacktrace logging cleanup/consolidation
