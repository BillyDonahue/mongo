...
UUID retrieveReshardingUUID(OperationContext* opCtx, const NamespaceString& ns) {
    ...
    uassert(/* TODO (copy from SERVER-52770) */ ErrorCodes::NoSuchKey,
...
public:
    ...
    public:
        ...
        void typedRun(OperationContext* opCtx) {
            ...
            auto serviceRegistry =
                repl::PrimaryOnlyServiceRegistry::get(opCtx->getServiceContext());
            auto service =
                serviceRegistry->lookupServiceByName(ReshardingCoordinatorService::kServiceName);
            BSONObj instanceId =
                BSON(ReshardingCoordinatorDocument::kReshardingUUIDFieldName << reshardingUUID);
            auto coordinator = ReshardingCoordinatorService::ReshardingCoordinator::lookup(
                opCtx, service, instanceId);
            uassert(ErrorCodes::NoSuchReshardCollection,
                    format(FMT_STRING("Coordinator: {}"), tojson(instanceId)),
                    coordinator);
