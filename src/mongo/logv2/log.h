...
#define LOGV2_IMPL(ID, SEVERITY, OPTIONS, FMTSTR_MESSAGE, ...)                          \
    ::mongo::logv2::detail::doLog(                                                      \
        ID,                                                                             \
        SEVERITY,                                                                       \
        ::mongo::logv2::detail::prepareOptions(OPTIONS, MONGO_LOGV2_DEFAULT_COMPONENT), \
...
#define LOGV2_OPTIONS(ID, OPTIONS, FMTSTR_MESSAGE, ...) \
    LOGV2_IMPL(ID, ::mongo::logv2::LogSeverity::Log(), OPTIONS, FMTSTR_MESSAGE, ##__VA_ARGS__)
...
#define LOGV2_INFO_OPTIONS(ID, OPTIONS, FMTSTR_MESSAGE, ...) \
    LOGV2_IMPL(ID, ::mongo::logv2::LogSeverity::Info(), OPTIONS, FMTSTR_MESSAGE, ##__VA_ARGS__)
...
#define LOGV2_DEBUG_OPTIONS(ID, DLEVEL, OPTIONS, FMTSTR_MESSAGE, ...)                        \
    do {                                                                                     \
        ...
        auto optionsMacroLocal_ = ::mongo::logv2::LogOptions::ensureValidComponent(          \
            OPTIONS, MONGO_LOGV2_DEFAULT_COMPONENT);                                         \
        if (::mongo::shouldLog(optionsMacroLocal_.component(), severityMacroLocal_)) {       \
            LOGV2_IMPL(                                                                      \
...
#define LOGV2_DEBUG(ID, DLEVEL, FMTSTR_MESSAGE, ...) \
    LOGV2_DEBUG_OPTIONS(ID, DLEVEL, LOGV2_DEFAULT_OPTIONS, FMTSTR_MESSAGE, ##__VA_ARGS__)
