...
TEST_F(MongosTopoCoordTest, AwaitIsMasterReturnsCorrectFieldTypes) {
    ...
    auto response = getTopoCoord().awaitIsMasterResponse(opCtx.get(), std::nullopt, std::nullopt);
...
TEST_F(MongosTopoCoordTest, IsMasterReturnsErrorInQuiesceMode) {
    ...
    // No topology version
    ASSERT_THROWS_CODE(getTopoCoord().awaitIsMasterResponse(opCtx.get(), std::nullopt, std::nullopt),
                       AssertionException,
                       ErrorCodes::ShutdownInProgress);
}
