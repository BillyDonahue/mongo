...
/**
 * True if a numeric "code" field indicates a state change. There are two
 * categories of state change errors: ShutdownError and NotPrimaryError.
 */
bool needsRewrite(ServiceContext* sc, const BSONObj& node, int& oldCode) {
    int intCode{};
...
boost::optional<BSONObj> rewriteDocument(const BSONObj& doc, OperationContext* opCtx) {
    ...
    if (okValue == 0 && needsRewrite(sc, doc, oldCode))
    ...
    if (const auto& we = doc["writeErrors"]; we.type() == Array) {
        ...
        for (auto ai = bArr.begin(); ai != bArr.end(); ++ai, ++idx)
            if (ai->type() == Object && needsRewrite(sc, ai->Obj(), oldCode))
    ...
    if (const auto& wce = doc["writeConcernError"]; wce.type() == Object) {
        if (needsRewrite(sc, wce.Obj(), oldCode))
